...
const debtData: DebtItem[] = [
  { name: 'Mortgage', height: '10%', color: 'bg-emerald-500', amount: 2500 },
  { name: 'Car Loan', height: '25%', color: 'bg-green-500', amount: 6250 },
  { name: 'Credit Card', height: '75%', color: 'bg-rose-500', amount: 18750 },
  { name: 'Student Loan', height: '40%', color: 'bg-amber-500', amount: 10000 }
]

export function LoanDebtChart({ value, percentageChange, isInView }: LoanDebtChartProps) {
  ...
  return (
    <div className="flex flex-col gap-4 rounded-lg border border-[rgb(30,30,30)] bg-[rgb(15,15,15)] p-6 shadow-sm lg:col-span-2">
      ...
      <div className="relative grid h-[200px] grid-cols-4 items-end gap-4 px-2 pt-4">
        {debtData.map((debt, index) => (
          ...
          >
            <div className="relative w-full flex items-end justify-center">
              <motion.div
                className={`w-full rounded-t ${debt.color} cursor-pointer relative`}
              ...
              {hoveredBar === index && (
                ...
                >
                  <div className="flex flex-col gap-1">
                    <span className="text-gray-100 font-medium">${debt.amount.toLocaleString()}</span>
                    <span className="text-gray-400">{((debt.amount / value) * 100).toFixed(1)}% of total</span>
                  </div>
              ...
              {/* Risk pulse for Credit Card */}
              {debt.name === 'Credit Card' && hoveredBar === null && (
                <motion.div
